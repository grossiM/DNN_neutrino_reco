...
| :---:        | :-----:         | :---:          | :----------------------------------------
| `output`     | `output-folder` | string         | Absolute path to the output DNN model.
...
| `input`      | `data-train`    | string         | Absolute path to the training HDF dataset.
...
| `training`   | `training-variables` | string    | The list of the training features. Different features are separated by `,`, each item in the list has to correspond to a column name from the input HDF datasets.
...
| `evaluation` | `data-eval`     | string        | Absolute path to the evaluation HDF dataset. The output HDF will inherit the name of the input, with addition of `.calibrated` to its base name.
