...
import handler_kinematics2 as kinematics2
...
class DataHandler():
    ...
    def appendQuadEqParams(self, flavour='mu'):
        a, b, c, delta = kinematics2.abcdelta(self.pdarray, flavour)
    ...
    def appendTwoSolutions(self, flavour='mu'):
        sol0, sol1, label = kinematics2.tag_solutions(self.pdarray, flavour)
    ...
    def appendEnergy(self, flavour):
        ...
        self.pdarray['v_'+flavour+'_predict_E0'] = kinematics2.calc_energy(self.pdarray[variables0].values)
        self.pdarray['v_'+flavour+'_predict_E1'] = kinematics2.calc_energy(self.pdarray[variables1].values)

    def calcCosTheta(self, variables, name):
        cosTheta = kinematics2.cos_theta(self.pdarray, variables)
    ...
    def appendMass(self, variables, name):
        ...
        self.pdarray[name+'_mass'] = kinematics2.calc_mass(px,py,pz,e)

###################################################################################### new part

    def calcpL(self, variables, name):
        pL = kinematics2.vector_manipulation(self.pdarray, variables)
        self.pdarray['pL'+name] = pd.Series(pL, index=self.pdarray.index)


    def appendSelectionCriteria(self, flavour='mu'):
        sel1, sel2, sel3, sel4, sel5 = kinematics2.tag_selectioncriteria(self.pdarray, flavour)
